...
import importlib
importlib.reload(resume_generator)
import numpy as np
import pandas as pd
...
"""
    ...
    with col3:
            ...
            st.write(
                "Make ATS Freindy Resume using your details."
            )
    with col4:
...
if selected == "Resume Builder":
    ...
    projects = st.text_area(
        ...
        placeholder="AI Placement Assistant\nCat vs Dog Classifier"
    )
    ...
    if st.button("Generate Resume"):
        ...
        pdf =resume_generator.create_resume(
            name=name,
            email=email,
            phone=phone,
            linkedin=linkedin,
...
if selected == "Resume Analyzer":
    ...
    if st.button("🔍 Analyze Resume"):
        ...
        if uploaded_file is None:
            ...
            resume_text = resume_analyzers.extract_text(uploaded_file)

            resume_text = resume_analyzers.clean_text(resume_text)

            job_description = resume_analyzers.clean_text(job_description)

            found_skills = resume_analyzers.extract_skills(resume_text)

            sections = resume_analyzers.detect_sections(resume_text)

            matched_skills, missing_skills = (
                resume_analyzers.match_job_description(
                    resume_text,
                    job_description
                )
            )

            ats_score = resume_analyzers.calculate_ats_score(
                matched_skills,
                missing_skills
            )
